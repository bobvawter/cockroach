...
func runTestPrettyData(t *testing.T, prefix string, cfg tree.PrettyCfg, matches []string) {
	for _, m := range matches {
		...
		t.Run(filepath.Base(m), func(t *testing.T) {
			...
			// walkabout --union Ast --reachable -d ./pkg/sql/sem/tree/ Statement Expr
			//
			// This created a new, top-level interface called Ast which is
			// implemented by  all types that implement Statement or Expr
			// implement, or which are transitively reachable.

			// DEMO: Dump stmt by visitor
			w := strings.Builder{}
			_, _, err = tree.WalkAst(stmt, func(ctx tree.AstContext, x tree.Ast) tree.AstDecision {
				w.WriteString(" (")
				w.WriteString(x.TypeId().String())
				return ctx.Continue().Post(func(ctx tree.AstContext, x tree.Ast) (ret tree.AstDecision) {
					w.WriteString(")")
					return
				})
			})
			...
			// DEMO: Dump stmt by treating it as a tree of abstract nodes.
			// We'll see some extra levels in this approach, since slices
			// exist as their own AstAbstract types.
			w = strings.Builder{}
			var dump func(tree.AstAbstract)
			dump = func(x tree.AstAbstract) {
				if x == nil {
					return
				}
				w.WriteString(" (")

...
func (p *planner) MemberOfWithAdminOption(
	...
	objDesc, _, err := p.PhysicalSchemaAccessor().GetObjectDesc(ctx, p.EvalContext(), p.txn,
		&roleMembersTableName, p.ObjectLookupFlags(true /*required*/, false /*requireMutable*/))
	if err != nil {
		return nil, err
	}
